...
LoginCheckNumber::LoginCheckNumber(std::string configPath)
    :EmailSenderConfig(configPath),
    MysqlServerConfig(configPath)
{
}

void LoginCheckNumber::operator()(const httplib::Request &request, httplib::Response &response)
{
    bool success = false;
    std::string msg;
    ...
    if(success)
    {
        ...
        {
            success = true;
            msg = "check number send to email success";
        }
        else
        {
            success = false;
